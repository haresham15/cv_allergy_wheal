...
try:
    ...
    from backend.services.segmentation import _load_sam, _is_wheal_shaped, WhealResult
...
def run_pipeline(min_score_floor=0.55, max_cands=300):
    blobs = blob_log(prep["l_clahe"], min_sigma=2, max_sigma=25, num_sigma=10, threshold=0.040)
    
    # Contrast scoring & Skin mask filtering
    scored_blobs = []
    for b in blobs:
    ...
    # Sort by true response contrast
    ...
    # Safe decoupled area limits
    min_area_px = max(15.0, min(config.SAM_MIN_MASK_REGION_AREA, config.MIN_WHEAL_AREA_MM2 * (ppm ** 2)))
    ...
    for i, center_pt in enumerate(candidates):
        ...
        for j in range(3):
            ...
            # Soft score floor instead of rigid 0.70 cutoff
            ...
            # Must be primarily on skin
            if np.logical_and(mask_binary, skin_mask == 0).sum() / area_px > 0.30:
                continue
...
print("\n--- Evaluating Pipeline with User Fixes ---")
for floor in [0.55, 0.60, 0.65, 0.70]:
    r = run_pipeline(min_score_floor=floor)
    print(f"Floor={floor:.2f} | Det={r['det']:2d}, TP={r['tp']:2d}, FP={r['fp']:2d} | Rec={r['recall']:.1f}%, Prec={r['precision']:.1f}%, F1={r['f1']:.1f}% | MAE={r['mae']:.2f}mm, IoU={r['iou']:.4f}, Dice={r['dice']:.4f}")
